...
public class MBeanRegistry {
    ...
    private Map<ApeMBeanInfo, String> mapBean2Path =
        new ConcurrentHashMap<ApeMBeanInfo, String>();
    
    private Map<String, ApeMBeanInfo> mapName2Bean =
        new ConcurrentHashMap<String, ApeMBeanInfo>();
     ...
     * node of this parent.
     */
    public void register(ApeMBeanInfo bean, ApeMBeanInfo parent)
        throws JMException
    {
        assert bean != null;
        String path = null;
     ...
     * @param bean
     */
    private void unregister(String path,ApeMBeanInfo bean) throws JMException {
        if(path==null)
            return;
        if (!bean.isHidden()) {
            try {
     ...
     * @param bean
     */
    public void unregister(ApeMBeanInfo bean) {
        if(bean==null)
            return;
    ...
    public void unregisterAll() {
        for(Map.Entry<ApeMBeanInfo,String> e: mapBean2Path.entrySet()) {
            try {
    ...
    protected String makeFullPath(String prefix, ApeMBeanInfo bean) {
     ...
     */
    protected ObjectName makeObjectName(String path, ApeMBeanInfo bean)
